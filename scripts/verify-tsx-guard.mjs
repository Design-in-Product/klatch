...
import { stripSource, regexLiteralEnd, REGEX_MAY_OPEN_AFTER } from './lib/strip-source.mjs';
...
// The source scanner that answers 'which bytes are code?' for every reading in this file moved
// to `lib/strip-source.mjs` in Round 259, with its whole Round 129-258 decision log. Theseus's
// Round 258 measured three implementations of that one question in the tree and established the
// direction: this one is the only reader not fooled by a regex literal containing an apostrophe,
// so it is the target and not the copy. Nothing about the scan changed in the move — the
// extraction is driven byte-for-byte against the pre-move reader in
// `scripts/probe-round259-…mts`, and the 213 checks below are the same 213.
