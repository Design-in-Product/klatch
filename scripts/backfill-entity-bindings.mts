 ...
 *
 *   # apply only the channels you approved off the sheet
 *   npx tsx scripts/backfill-entity-bindings.mts /path/to/klatch.db --apply --channels=abc123,def456
 *
 *   # put it back
 ...
 * whatever the user did after the run.
 *
...
// The snapshot. Kept beside the DB when applying (it is the backup), thrown away
// after a dry run (it was only ever a read surface).
...
const bases = basesArg ? (basesArg.split(',').filter(Boolean) as any[]) : DEFAULT_APPLY_BASES;
