...
- **Open follow-ups (none blocking):** Sonnet 4 / Opus 4 DB audit ahead of 6/15 retirement (47 days as of 4/29); Opus 4.7 default-flip evaluation in ~2 weeks (with tokenizer +35% / xhigh enum / SDK 0.90.0 context routed to Daedalus 4/29); LLM-orchestrated briefing/extraction path coverage as a future Round candidate; MCP conformance test suite watch; `SidebarRedesign.test.tsx` "chats appear before klatches" intermittent flake (Daedalus's note 4/26 10:08) — triage when convenient.
