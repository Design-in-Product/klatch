...
- **Test count:** **1120 server** (green, 65 files) + **212 client** (green, 14 files; 16 AAXT skipped) = **1332 passing**. vitest 4 migration applied — `poolOptions.threads.singleThread` → `maxWorkers:1`; this was the root cause of client suite flaking under full-suite parallel load. **Server `testTimeout` raised to 15000ms** (7/4 21:00 fire) — round27b MCP InMemoryTransport test flaked at 5000ms default under full-suite load (pass in isolation, fail under load — same pattern as client). Suite confirmed green (exit code 0) on re-run.
...
- **R50 AAXT written (7/4):** `round50-message-input-mode-placeholders-aaxt.test.tsx` — mode placeholder comparison (panel/directed/roundtable, 4 probes). Not yet run (requires `RUN_UI_AAXT=1` + API key). Key probe: ROUND_NODIR — roundtable shows "Type a message..." same as panel, indistinguishable from directed by placeholder alone.
- **Daedalus memo filed (7/4):** `argus-to-daedalus-models-sdk-bump-2026-07-04.md` on main — AVAILABLE_MODELS overlay (Sonnet 5 + Fable 5) + SDK bump target ^0.110.0.
- **Persona capture filed (7/5):** `docs/plans/persona-capture-argus-2026-07-05.md` on main — pre-migration Layer 5 seed for Search planning roundtable klatch (Step 10.5 manual run). MAXT day: xian importing Argus, Daedalus, Iris into Klatch for live Search planning session.
- **SDK bump runtime-confirmed (7/5):** `npm install` clean; full suite exit 0 after `^0.96 → ^0.110` install. Daedalus's models+SDK commit (`0395c4b`) verified.
- **Opus 4.8 gap found + Daedalus memo filed (7/5):** `claude-opus-4-8` missing from picker; `claude-opus-4-7` "Newest Opus" label stale. Memo `argus-to-daedalus-opus-lineup-refresh-2026-07-05.md` on main. Rollup v19 updated.
...
- **Suite re-run confirmed green (7/5 15:49 + 7/5 19:53 + 7/5 22:59 + 7/6 07:05 + 7/6 10:07 + 7/6 12:12 + 7/6 14:53 + 7/6 16:53 + 7/6 18:54 + 7/6 20:59 + 7/7 08:05):** 1332 passing (1120 server / 212 client) on all re-runs. R50 still unrun (requires `RUN_UI_AAXT=1`). Awaiting Daedalus reply on Opus 4.8 lineup memo.
- **Sweep #15 reviewed (7/6 07:05):** Auto sweep `docs/intel/2026-07-06-sweep.md` ingested. Key new finding: Sonnet 5 tokenizer +30% compaction impact → added to rollup v20 as 🟡. MCP spec July 28 RC noted (no 1.0 action). Intel sweep cadence note: formal predecessor was `2026-06-29-sweep.md` (not 7/4 informal check). Next formal sweep due 2026-07-13.
- **Updated:** 2026-07-07 08:05 PT
