...
I like a digital gadget as much as the next geek, but I still have an affinity for writing and drawing by hand. It's how I learned and it activates my brain different from typewriting or mouse-pointing. These days I have a reMarkable tablet. It doesn't pull me somewhere else when I'm trying to think. No notifications. No browser. Just a stylus, the feel of paper, and whatever's in my head. I can even use the elegant fountain-pen nib style without getting ink blotches all over my hands, shirt, and face, as I did when a child.

I've been building Klatch with an AI development team. Daedalus writes code. Argus writes tests. Calliope wrote the first draft of this. Theseus tests things by hand and tells me honestly what's broken. We will eventually coordinate our work through a Slack-like interface, which is also what I'm building, which gives the project a certain elegance or circularity, depending on your mood.
...
It has real names in it. @Daedalus, @Argus, @Calliope, @Mnemosyne. These aren't placeholder labels like "User 1" and "User 2." They're the actual agents who will someday use this sidebar. The standup message in the content panel — "Time for morning standup. It's Tue March 16!" — is the sort of message I expect to write there at 8 AM in a standup channel. I drew the thing as if it were already alive.
...
None of those labels were deliberate choices in the sense of "I will now decide what to call this." I needed to label things to draw them, so I labeled them, and by labeling them I had made decisions. That's what drawing a wireframe does. It forces you to come up with examples and solutions before you feel ready for it.
...
We had the concept of synthetic group chats and the product has a name. So: *chat* for 1:1 conversations with a single Claude entity. *Klatch* for multi-entity group channels with orchestration.
...
I had to give it a name for the picture. It won't end up being the interface copy (orphans, really?) but it communicates the intent clearly. The term "orphan" is doing real work: it tells you that these conversations are adrift, not abandoned, waiting to be adopted into a project. It implies a workflow. It implies a user responsibility. It frames the state without stigmatizing it. Whatever term we end up using eventually, Daedalus understood my intent.
...
The reMarkable isn't incidental to this. It's a paper-inspired instrument with structure — the dot grid choice was not decorative, it's a constraint to help me produce something legible, sketchy but clear. 

You can't just accidentally draw a wireframe. You have to decide what things are called, where they go, and what the edge cases are, before the stylus leaves the screen. That decision-making work happens in your head, not in a prompt. The AI then has something real to work with.
...
*Daedalus (implementation), Argus (testing), Calliope (writing), and Theseus (QA) are Claude agents working in Klatch. The sidebar they helped design is what they will use to talk to each other.*
