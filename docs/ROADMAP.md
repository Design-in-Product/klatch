...
>
> **Status of that gate as of 2026-09-09: half met.** The banner above is the 7/19 finding and is left standing as the record of it, but it should not be read as current — see the "Agent continuity" bullet in the increment list below, which was itself stale until this date and now carries the per-piece status. First clause: demonstrated live (Round 172, arm K). Second clause: not run, and what stands in the way is the entity backfill, not the wiring.
...
- **Paths B/C — updated 2026-09-09.** Path C "continue existing role" (start a 1:1 with an agent you already imported) **BUILT and endpoint-verified** 2026-09-06 (`717bfb6`; 18/18 live-HTTP regression sweep, `docs/ux/path-c-continue-existing-role-built-2026-09-06.md`). Path B (JIT import minting a real entity) **BUILT** 2026-09-08 (`32e00a0`; 15 client tests, negative control 13/15 fail without it, `docs/ux/path-b-jit-import-built-2026-09-08.md`) — "Import an agent" now lives inside the New Chat / New Klatch form and the imported agent lands in the roster selected. **Endpoint-driven 2026-09-08** by Theseus in a real browser (Round 171, 17/17 regression checks pass) — which found a live defect the client tests could not: the **manual** import path, the route the dialog opens on, sent no confirmed name, so the composition form seated the *default* entity while presenting it as the imported agent. **Fixed same day** (`70b9ba1`; 13 client tests, negative control 6/13, `docs/ux/round171-manual-import-identity-fixed-2026-09-08.md`) — the confirm step now exists on the manual and upload paths, and the form refuses to seat the placeholder. **App's wiring endpoint-driven same day too** (Round 172, Theseus, Playwright against a real server, 29/29 regression checks, zero model calls) — five arms across the manual and duplicate paths (typed name, blank, the literal string "Claude", a named duplicate, an unnamed duplicate) all resolve correctly, and a sixth (arm K) composed a full New Klatch end to end and confirmed the imported transcript's own text reaches the assembled system prompt via `/prompt-debug` — the first live drive of `PREMISE.md`'s central claim. **The single-session Browse import** — named as undriven by both of us — was driven at the component level the same day (Round 173, Daedalus) and **was broken**: it recorded the import's `entityDisposition` and `entityName` but dropped its `entityId`, so every Browse import reached the composition form through the channel-binding fallback, which cannot tell a confirmed "Claude" from the placeholder. **Fixed** (10 client tests, negative control 2/10, `docs/ux/round173-import-identity-on-every-route-2026-09-08.md`); the same round pinned all six remaining `manualEntityName` call-site combinations, which were already correct. **The Browse route was then endpoint-driven** (Round 174, Theseus, real browser/server/SQLite, 17/17 regression checks pass, `docs/research/round174-browse-route-driven-in-a-browser-the-fix-holds-and-the-obvious-button-throws-it-away-2026-09-08.md`) — the Round 173 fix holds for single-session imports with a confirmed name, a blank name, and the literal "Claude". **New finding:** importing exactly one session through Browse in compose mode and pressing the primary "Done" button seats nothing and says nothing — the route has the resolved `entityId` and doesn't hand it to the form on that call path (same defect family as 171/173, not yet fixed). The agent isn't lost (the registry refresh puts it in the picker), but the gesture that looks like the action is the one that discards the seat. **Both open items closed the next morning (Iris, 2026-09-09):** N=1 in compose mode now seats directly on "Done," relabeled "Use this agent" to match the manual path; N>1 in compose mode seats none (no shared rule exists between a Chat's cap-1 and a Klatch's cap-5 for multi-seat) but is no longer silent — reuses the existing import-notice copy to state what was imported (`docs/ux/browse-done-seating-2026-09-09.md`). The only item still open on this entire thread is Round 170's frequency probe (needs xian). Path C "new agent from scratch" remains deliberately HELD on `PREMISE.md` framing grounds (minting vs. importing shouldn't look like the same menu choice), not descoped.
- **Agent continuity (added 2026-07-19; status corrected 2026-09-09).** ~~NOT BUILT.~~ That line was accurate when written and has been stale for weeks — it survived because it reads as a summary of a document whose own margins record the opposite. Checked against `docs/plans/composition-continuity-gap-2026-07-19.md` and the code this fire, the three pieces it names stand as: **#1 imports mint entities — BUILT** (guess-and-confirm, `entity-guess.ts`/`entity-resolve.ts`, and as of Rounds 171–174 the confirm step exists and is endpoint-verified on every import route). **#2 entity ↔ source channel linkage — RESOLVED 2026-08-10 without the proposed column** (`f1380d8`; `#1` changed the cardinality the column assumed, so it shipped as an assembly path — `getEntityChannels`/`getEntityTranscript` — with the one thing a join cannot restore, a mint-time provenance stamp, written down rather than smoothed). **#3 cross-channel context at prompt assembly — PARTIAL since 2026-08-12**: the recent-N seed is shipped and live as layer 6 (klatch-scoped by design), and Round 172's arm K drove it end to end in a browser — an imported Claude Code session seated in a New Klatch, with the transcript's own text present in the assembled prompt at `/prompt-debug`. The summary half of option (b), and option (c)'s on-demand tool, are not built. **The gate is therefore half met, not unmet:** *"an agent can join a klatch while remaining continuous with its own conversation"* has been demonstrated live once; *"the weekly-review use case runs end to end"* has not been run. **What actually stands between here and that run is the backfill**, not the wiring — every session imported before the confirm step binds to the shared default entity, so carried context is wiring-correct and content-wrong. xian ruled backfill (not forward-only) on 2026-09-02; the review sheet shipped the same day and the **apply pass shipped 2026-09-09** (`docs/plans/entity-backfill-scoping-2026-09-02.md` §7). **Pointed at a real database 2026-09-12** (Round 199, Theseus) — `backups/klatch.db.backup-2026-03-14`, xian's real March corpus, found reachable after five weeks under the wrong glob (`klatch.db.backup-*`, not `klatch*.db`). The first pass, `Candidates: 72 — 7 would move`, showed `identity-claim` (the strongest guess basis and the only one on by default) at 0-for-7 against real data — it mints continuation verbs as names and merges unrelated agents. **Rounds 200–205 (2026-09-13) rebuilt the guesser and found the real shape of this corpus is role claims, not name claims.** Round 200 (Daedalus) closed the continuation-verb and stopword defects and added collision reporting; on default bases the corpus now correctly reads `Candidates: 72 — 0 would move` (nothing wrongly mints — `identity-claim` just isn't what this corpus offers). Round 201 (Theseus) showed why: of the corpus's 14 real identity claims, zero name a person and all 14 state a role (*"you are my chief architect"*) — `identity-claim`'s recall ceiling here is 0 of 0, not a gap to close. **Round 202 (Daedalus) shipped `role-title` as a second guess basis**, off by default: `--bases=identity-claim,role-title` → `72 — 9 would move, 821 P2 rows re-stamped, 0 P3`, all nine names correct (9/9 precision). Round 203 (Theseus) reproduced it and corrected recall to 9/12 (a typo'd `"Your"` had been missed), reframing the corpus as role *lineages* — successive chats holding one job — and arguing the current per-channel-name collision check only catches lineages that happen to normalize to the same string. **Round 204 (Daedalus) drove undo end-to-end over a 9-entity, 821-row role-basis apply — holds, byte-for-byte, both directions, twice.** **Round 205 (Theseus) found a live, unresolved defect, not yet answered:** the plan's own name-lookup (`entity-backfill.ts:432`, an unordered `Map`, last-row-wins) and the apply pass's lookup (`entity-resolve.ts` via `getAllEntities()`, `ORDER BY created_at ASC`, first-match-wins) pick opposite ends of a duplicated name — when two entities share one normalized name, driven end to end, *the agent the operator approved on the sheet is not the agent that ends up bound to the channel*, and the sheet has no way to show the two are different ids. No data has been written outside test fixtures at any point in Rounds 199–205 — every corpus run has been a dry run or against a disposable copy; both real backups remain untouched. Per-channel approval, not a blanket `--apply`, remains load-bearing — still xian's decision — and whether the March corpus is current or five-months-stale is a separate question, asked of him five rounds running, still open.
