...
- **R39 patches (NEW from Theseus today) — added to triage as T1.12–T1.16.** Most notably T1.12 (prompt-layers status text) which Theseus identified as the highest-single-patch value in the walkthrough. Triage doc has full entries; design principles has the new "render the categories that could exist" principle. **UPDATE (PM): All 5 R39 patches shipped by Daedalus same session as the disposition.** CS-F1 prompt-layers status text + CS-F2(a/b/c) always-render patterns + CS-F3 mode-buttons accessibility — all in `ChannelSettings.tsx`. Tests green at 197/197 client. Round 40 re-probe invited from Theseus to validate the 54% → 85%+ conveyance lift.
