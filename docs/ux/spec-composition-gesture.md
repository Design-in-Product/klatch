...
- **Path B (just-in-time import) — ~~SCHEDULED~~ BUILT 2026-09-08** (`32e00a0`, Daedalus). "Import an agent" now sits inside the New Chat / New Klatch form; completing the import returns the minted agent to that form, selected, instead of navigating away. Details: `docs/ux/path-b-jit-import-built-2026-09-08.md`. **The sequencing below named the wrong dependency** — the real blocker was Iris's import confirm step (cleared 2026-09-02), not continuity #2–#3; before it the client sent no entity fields at all. Two notes for the record: the affordance had to sit *outside* the picker's `entities.length > 0` gate, since a registry with nothing in it is exactly when import should be the front door; and entities were loaded once at mount and never refreshed by an import, so a newly minted agent was invisible to the picker until reload — a defect that predates this path. Originally scheduled sequenced after continuity increments #2–#3. Its blocker is gone: before increment #1 (imports mint entities, shipped 2026-08-08, `823054f`) an inline import would have bound the agent to the shared default entity and delivered an agent with no identity or context — the exact broken thing. Imports now mint a real entity via guess-and-confirm, which also settles Iris's open question on this path (JIT one-offs **name on import**, they do not arrive nameless).
  - **Round 171 correction (2026-09-08).** The paragraph above is right about the blocker and wrong about the coverage. Theseus drove Path B in a browser the same day it shipped and found that *"imports now mint a real entity via guess-and-confirm"* held only for the **Browse** route. The manual path-input route — the one the dialog opens on, and therefore the one a user reaches first — sent no `entityName` at all, so resolve returned `default`, the channel bound `DEFAULT_ENTITY_ID` as a placeholder, and the composition form seated a chip reading "Claude" for a session whose own identity never reached the assembled prompt. That is the §11a broken thing, still live on one route, now asserting an agent had been seated. Fixed same day (`70b9ba1`): the confirm step now exists on the manual and upload paths, and the form's fallback refuses to seat the placeholder — reporting *"the session didn't name an agent"* rather than naming the wrong one. Findings: `docs/research/round171-path-b-driven-in-a-browser-the-default-way-in-seats-the-default-entity-2026-09-08.md`. **The lesson worth keeping: "the import path mints identities" was a claim about one route stated as a claim about the feature.**
- **Path C → "Continue existing role" — ~~SCHEDULED~~ BUILT 2026-09-06** (`717bfb6`, Daedalus Round 160), reviewed and ruled by Iris the same day (both copy/sizing calls kept as shipped; checkbox/radio a11y smell fixed). Details: `docs/ux/path-c-continue-existing-role-built-2026-09-06.md`. Note for the record: *"the picker already enumerates entities"* was true only for klatches — the picker was gated to `newType === 'klatch'`, so every 1:1 chat was created against the shared default entity. The server already accepted a one-agent chat roster; the client never sent one.
