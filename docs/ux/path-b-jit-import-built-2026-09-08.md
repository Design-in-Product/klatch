...
## Iris — the three items routed here, ruled (2026-09-08, STOP fire)

**The two copy calls stand.** "Import an agent" + "Bring one in from Claude Code or claude.ai — it arrives with its conversation" and "Use this agent" as the compose-mode completion button — both read correctly against `PREMISE.md` and against §11's HELD/BUILT split. Not overruling either.

**The shape question — resolved, not designed around.** Daedalus asked whether the affordance sitting outside the `entities.length > 0` gate reads as an orphan on a fresh install with zero agents, since he hadn't seen that state on a real screen. Checked the actual precondition rather than the screen: `entities.length === 0` is not a reachable state against a real `klatch.db`. `db/index.ts:84` seeds a default entity ("Claude") on schema creation, and `db/index.ts:347-351` re-checks and backfills it on every startup even against an existing DB — both read at source this fire, not recalled. `getAllEntities()` (`db/queries.ts:351`) has no filter that would exclude it from the client's `entities` prop. So a real install always has at least one agent, the picker always renders, and the import button never appears as the lone element in that section. The component-level test that renders `entities={[]}` (`composition-path-b-jit-import.test.tsx:89`) is a legitimate defensive-rendering guard, not a description of a state a user will ever see — left it in place, no code change needed here.

**The two failure notices stand, with one fix landed alongside them.** "Imported — the roster is full (5). Remove an agent to seat it." and "Imported, but no agent came back with it — pick one from the list." (plus the Browse-route twin, "Imported — the session didn't name an agent. Pick one, or re-import with a name.") are honest and specific; not rewriting them. **What did need a fix:** Theseus's Round 172 screenshot (`round172-shots/03-B2-blank-unidentified.png`) showed the unidentified-import notice stacked directly above the chat picker's evergreen "Optional — leave empty to start with a new assistant" hint — two sentences naming "optional"/a choice in the same ~200px, one transient and specific, one standing and generic. Daedalus's Round 173 fix routes the Browse import through the same notice function, so the same stacking now happens on both routes, not one. Fixed by suppressing the generic hint while `importNotice` is showing (`ChannelSidebar.tsx`, chat picker section) — the notice already tells the user a choice exists; the second, differently-worded line was repeating the point rather than adding to it. New test pins the suppress/reappear behavior (`composition-path-b-jit-import.test.tsx`, "does not also show the generic 'Optional' picker hint while a notice is up"). Verified: server 1561/1561 (unchanged), client 306/306 (+1), typecheck clean ×3 workspaces.
