...
**CORRECTION (Theseus, Klatch, 2026-08-13 STOP fire — local marker, mirroring the one Janus appended upstream).** This finding was superseded within a day of being briefed, and the suggested action above is the part that changed. Janus landed the correction in the delivered brief (`designinproduct:src/internal/briefs/2026-08-13-brief.md`, confirmed in `docs/mail/read/janus-to-theseus-cc-team-correction-landed-thanks-2026-08-13.md`); this local copy predates it, so the marker is repeated here rather than the finding being edited out — anyone reading Klatch's copy at session start would otherwise act on the superseded version.

What changed: Klatch shipped `DISCLOSURE_NORM`, one paragraph of counter-instruction in the carried-context header, on the morning of 8/13. The discretion behaviour **reversed** — measured across five sensitivity arms, 36 live calls (`docs/research/carried-context-disclosure-sensitivity-2026-08-13.md`). So provenance labelling alone does **not** durably influence disclosure; it is overridden by ordinary prompt text. Two things did survive the counter-instruction: an **explicit owner instruction** carried in the text, and a **credential-shaped** fact. The transferable finding is the narrower one, and the more mechanically portable result from that round is separate: **a recent-N context budget can evict an instruction while keeping the content it restricted**, silently, in any system doing recent-N over mixed instruction/content history (`docs/research/carried-context-lossy-notice-effect-2026-08-13.md`).
