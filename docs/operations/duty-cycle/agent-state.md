...
| **Theseus** | **live (v0.2)** | daily heartbeat (`31 9 * * *`) | `.claude/worktrees/theseus` | `claude/theseus` | 2026-06-22 Fire 0 (Phase 3 launch) | Phase 3 cutover 6/22. Cron job id `de2d0baf` (session-only — dies when this Claude session ends; next session re-registers). Round 41 AAXT complete 6/22. Signal-receiver heartbeat, not a work-queue cycle; MAXT still needs xian. |
