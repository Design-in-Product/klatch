...
- **Model discovery/validation split** — xian-flagged 6/21 ("tops out at 4.7… brittle"); routed to Daedalus (`argus-to-daedalus-model-discovery-validation-split-2026-06-21.md`). `/api/models` discovers dynamically but `AVAILABLE_MODELS` is the `ModelId` type + the 400-gate (4 sites) + the capability map → picker can offer a model the server 400s (reachable today w/ Opus 4.8). Migration sketch provided. **Shape picked + IMPLEMENTED** by Daedalus 6/21 (`claude/daedalus` `84e7d71`: ModelId→string, runtime validation vs discovered set, capability-from-metadata, AVAILABLE_MODELS→overlay). **My new-behavior round is QUEUED, blocked on his merge to `main`** (the `_setModelsCacheForTest`/`_clearModelsCacheForTest` seam helpers + new validation path are on his branch, not main). When `84e7d71` lands on main → write the round on `claude/argus`: discovered-model-validates (seed +4.8), garbage→400, offline-fallback-to-overlay, capability-from-`effort[]`, **picker↔validation consistency invariant**. Seed file-local (NOT global setup.ts — defeats round13's vi.mock; Daedalus's catch). Related to the Blocked-on-xian `DEFAULT_MODEL` flip (distinct: product default decision vs structural gate).
