...
- [ ] **Flaky-test triage — SidebarRedesign DOM-order** — Daedalus **CORRECTED** his root-cause 6/21 (`sidebar-rootcause-correction`): NOT a query/data bug — the client structurally guarantees chats-before-klatches (separate arrays per project, `ChannelSidebar.tsx:152-155`). It's **TEST-SIDE timing** (synchronous `getAllByRole` returns `-1` before the render settles; ~1-in-5–1-in-10 isolated, "6856ms" smell). **UNBLOCKED, mine — TOP next-fire priority.** Fix: harden to settled-render queries (`findBy*`/`waitFor`, or ensure the project accordion is expanded before indexing). Part of the **client-suite-fragility cluster** — one hardening convention closes this flake, shrinks the latent 0–3/run baseline, AND un-parks `composition-picker-extended.test.tsx` (same family as the ImportDialog flake).
