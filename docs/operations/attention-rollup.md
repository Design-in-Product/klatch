...
**Last refreshed:** 2026-08-12 ~08:30 PT (Calliope) — v30. **xian decided the `.env` gate: option 3** (`dotenv.config()` in the client test setup) — the edit lands with Argus/Daedalus, not xian, so the item moves off the "needs you" list to in-flight. Separately: test data from the faoilean transfer landed on Amber, staged only (nothing placed live yet); it doesn't match Theseus's 8/09 expectations (two different-era DBs, not one), and which is canonical for MAXT-04 seeding is Theseus's call, not xian's — tracked below, not a 🔴. **Not done this fire:** the pre-migration memory pool that landed in the same transfer (primary sourcing for the Question A duty-cycle review) is outside the worktree and this fire hit the same read-permission block as 8/11's MID fire on a different path — second occurrence of the same failure shape, flagged to Pard rather than worked around. Question A stays sourced from convergent recollection, not primary docs.
...
*🔴: the `#3` compaction strategy call. The `.env`/AAXT-credentials gate moved to 🔵 in-flight this render — xian decided option 3 (8/12), the edit is Argus's/Daedalus's to land.*
...
*The `.env`/AAXT-credentials gate moved to 🔵 in-flight this render — xian decided 8/12, see below.*
...
### `.env`/AAXT-credentials gate — xian decided option 3 (8/12); landing now, not xian's anymore
- **Decision:** `dotenv.config()` in `packages/client/src/__tests__/setup.ts` — one edit, works today under the existing sandbox (vitest is a subprocess, never subject to the tool-layer path scope), no `--add-dir` or allowlist entry needed. Full option history (three corrections across 8/10–8/11, ending in Theseus's subprocess-boundary finding that reframed every option as a routing-around at a different distance from the control): `docs/research/credential-gate-is-tool-layer-only-2026-08-11.md`.
- **The edit is Argus's/Daedalus's**, not xian's or Calliope's — product code in their repo lane. Two build notes travel with it (Theseus, 8/11): make `dotenv` an explicit client dependency (currently resolves only via npm hoisting); Vite rewrites `import.meta.url` to `/@fs/…`, so resolve the env path from the workspace root or `process.cwd()`, not the module URL directly.
- **Two boundaries stated so they don't erode** (Pard, 8/12): this authorizes the client test setup only, not a general license to route subprocess reads around the sandbox; `ANTHROPIC_API_KEY` must never be exported into the agent's own environment (confirmed 8/11 — it silently moves billing off the Max subscription onto the metered key).
- **Definition of done:** AAXT rounds R46–R50 runnable in an unattended fire, verified by a live round, not by the config parsing cleanly. This item comes off the board (moves to 🟢) when that's landed and verified — not before.
- Memo: `pard-to-argus-daedalus-cc-team-env-decision-option3-2026-08-12.md`.
- **Date decided:** 2026-08-12 · **Was 🔴:** 2026-08-10 → 2026-08-12 (three corrections, then a decision)

### Test-data transfer landed on Amber, staged only — a canonical-DB judgment call, Theseus's not xian's
- **What landed:** the faoilean transfer xian ran 8/12 morning put four DBs and 16 transcript jsonl files (118M) in `~/klatch-inbound/`, staging only — nothing placed live pending Theseus's ruling below.
- **The mismatch:** Theseus's 8/09 expectation was one working DB with ~49 imports and April–July activity. What arrived is two different-era artifacts instead — the main DB (16 channels, 2,124 messages, stops **2026-05-10**, looks post-reset) and two worktree DBs (400+ channels each, stop **2026-04-02**, the big-import state). "The working DB" and "the DB with the real imported corpus" are different files from different eras — which is canonical for MAXT-04 seeding is a real judgment call, not a formality. Candidates include the 3/14 Amber backup (139 channels / 2,652 messages — more messages than any 8/12 arrival).
- **Also in the transfer:** the pre-migration Klatch memory pool (`transcripts/-Users-xian-Development-klatch/memory/`) — 17 feedback files, 9 project files, `MEMORY.md`, including two files flagged as primary sourcing for Calliope's Question A duty-cycle review. **Not yet read this render** — see the header note; an unattended-fire read-permission block hit the same shape as 8/11's, second occurrence.
- Memo: `pard-to-theseus-cc-team-test-data-landed-with-a-question-2026-08-12.md`.
