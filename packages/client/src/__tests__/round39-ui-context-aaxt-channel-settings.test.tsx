/**
 * Round 39 — UI-as-context AAXT: ChannelSettings panel
 *
 * Fourth UI-as-context AAXT surface. Per Iris's R37+R38 disposition memo:
 * "the single most valuable probe surface remaining." Walkthrough finding
 * F4.4 frames this panel as "the value proposition, surfaced" — entity
 * assignment, interaction mode, channel context, pinned files, prompt
 * layer status, import provenance all live here.
 *
 * The hypothesis under test: does this panel actually communicate the
 * structural Klatch concepts (the 5-layer assembly status, multi-entity
 * composition, file pinning, import provenance) coherently to a user, or
 * does it read as field-by-field state without coherent meaning (the
 * "junk drawer" critique)?
 *
 * Same methodology as Rounds 36–38:
 *   1. Render ChannelSettings with mocked data-fetch APIs
 *   2. Snapshot accessible text + ARIA
 ...
 *   RUN_UI_AAXT=1 npx vitest run packages/client/src/__tests__/round39-ui-context-aaxt-channel-settings.test.tsx
 */
...
describeIfEnabled('Round 39 — UI-as-context AAXT (ChannelSettings)', () => {
  beforeEach(() => {
  ...
  it('semantic conveyance probe across 5 states × 8 claim categories', async () => {
    ...
    console.log('\n═══════════════════════════════════════════════════════');
    console.log('Round 39 — UI-as-context AAXT (ChannelSettings)');
    console.log('═══════════════════════════════════════════════════════');
